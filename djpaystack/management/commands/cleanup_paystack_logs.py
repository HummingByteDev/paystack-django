...
from djpaystack.models import PaystackTransaction, PaystackWebhookEvent
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        webhook_events = PaystackWebhookEvent.objects.filter(
            created_at__lt=cutoff_date, processed=True  # Only delete processed events
        )
        ...
        if webhook_count > 0:
            ...
            if not dry_run:
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ Deleted {webhook_count} webhook events"))
        else:
            self.stdout.write("No webhook events to delete")

        # Clean transactions (if not webhooks-only)
        if not webhooks_only:
            transaction_query = PaystackTransaction.objects.filter(created_at__lt=cutoff_date)

            # Filter by status if failed-only
            if failed_only:
                transaction_query = transaction_query.filter(status__in=["failed", "abandoned"])
            ...
            if transaction_count > 0:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Deleted {transaction_count} transactions")
                    )
            else:
