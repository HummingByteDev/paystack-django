...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not webhook_secret:
            self.stdout.write(self.style.ERROR("WEBHOOK_SECRET not configured in settings"))
            return
        ...
        try:
            ...
            # Or use convenience methods for common events
                ...
                response = tester.send_charge_success(
                    reference=options["reference"], amount=options["amount"], email=options["email"]
                )
                ...
                response = tester.send_subscription_create(email=options["email"])

            elif event_type == WebhookEvent.TRANSFER_SUCCESS:
                response = tester.send_transfer_success(amount=options["amount"])
